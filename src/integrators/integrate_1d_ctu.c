 ...
 */
  ...
  if (CoolingFunc != NULL){
    for (i=il+1; i<=iu; i++) {
      cc_pos(pG,i,js,ks,&x1,&x2,&x3);
      coolfl = (*CoolingFunc)(Wl[i].d,Wl[i].P,(0.5*pG->dt),(x1-0.5*pG->dx1),x2,x3);
      coolfr = (*CoolingFunc)(Wr[i].d,Wr[i].P,(0.5*pG->dt),(x1+0.5*pG->dx1),x2,x3);
 ...
 */
  ...
  if (CoolingFunc != NULL){
    for (i=is; i<=ie; i++) {
      cc_pos(pG,i,js,ks,&x1,&x2,&x3);
      coolf = (*CoolingFunc)(dhalf[i],phalf[i],pG->dt,x1,x2,x3);
